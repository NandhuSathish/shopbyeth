            ...
            if (orders.length !== 0) {
                orders.forEach(order => {
                    getTransactionStatus(order.rewardTxHash).then(status => {
                        const rewardCoinAmount = ((order.totalRewardableAmount - order.discount) * 1 / 100)
        ...
        Orders.aggregate([
            ...
            if (orders.length > 0) {
                for await (const order of orders) {
                    let updateBody = {
                        $inc: {
                            totalRewardableAmount: { $round: [ order.products.orderStatus === orderStatus.orderDelivered ? order.products.amount : 0, 6 ] }
                        },
                        $set: {
...
function mintRewardPoints() {
    try {
        Orders.aggregate([
            { $match: { rewardStatus: { $eq: 0 }, paymentStatus: { $eq: paymentStatusUtils.paymentSuccess }, products: { $not: { $all: [{ $elemMatch: { returnStatus: { $ne: 1 } } }] } } } }
        ]).then(async orders => {
            if (orders.length > 0) {
                let orderIndex = 0
                while (orderIndex !== orders.length) {
                    ...
                    if (order.total != order.discount) {
                        console.log(`Started mint function for Order: ${order._id}`)
                        let reward = ((((order.totalRewardableAmount - order.discount) * 1) / 100))
